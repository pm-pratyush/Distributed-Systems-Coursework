...
void generateMultibrotGridMPI(int N, int M, int D, int K, int rank, int size, vector<vector<int>> &grid)
{
    ...
    int s_idx = rank * pointsPerProcess;
    int e_idx = (rank == size - 1) ? totalPoints : s_idx + pointsPerProcess;
...
int main(int argc, char *argv[])
{
    ...
    if (rank == MASTER)
    {
        ...
        for (int i = 1; i < size; ++i)
        {
            ...
            int s_idx = i * pointsPerProcess;
            int e_idx = (i == size - 1) ? totalPoints : s_idx + pointsPerProcess;
